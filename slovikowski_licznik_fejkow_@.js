// ==UserScript==
// @name         Zbieranie fejków z wiosek v1
// @namespace    slovik-
// @description  Zbieranie fejków z wiosek o podanych koordach
// @match        https://*.plemiona.pl/game.php?village=*
// @version      1.0
// @author       Slovik
// @grant        none
// ==/UserScript==
...
(function () {
  ...
  // Create new quickbar item
  const quickbarItemHtml = `
      <li class="quickbar_item">
        <span>
          <a class="quickbar_link" data-hash="custom-script-link" href="#">
            <img class="quickbar_image" data-src="https://dspl.innogamescdn.com/asset/c1748d3c/graphic//buildings/script.png" alt="" src="https://dspl.innogamescdn.com/asset/c1748d3c/graphic//buildings/barracks.png">
            Zbieranie fejkow
          </a>
        </span>
      </li>
  ...
  const quickbarContents = document.querySelector(
    "#quickbar_contents ul.menu.quickbar"
  );
  quickbarContents.insertAdjacentHTML("beforeend", quickbarItemHtml);

  // Add click event listener to the new quickbar item
  const customScriptLink = document.querySelector(
    'a[data-hash="custom-script-link"]'
  );
  customScriptLink.addEventListener("click", runScript);
  ...
  function runScript(event) {
    ...
    while (true) {
      ...
      if (pattern.test(inputData)) {
        ...
        alert("Nieprawidłowe coordy, wejście: '111|111 222|222 333|333'.");
      }
    }
    ...
    if (inputCorrect) {
      ...
      async function fetchVillageData(villageId, coordinates) {
        ...
        try {
          ...
          const nicknameCounts = Array.from(attackElements).reduce(
            (counts, element) => {
          ...
          console.error(
            `Brak atakow dla wioski o ID ${villageId}, koordy ${coordinates}.`
          );
          return null;
        }
      }
      ...
      fetch(url)
        ...
        .then(async (body) => {
          ...
          UI.Notification.show(
            "https://help.plemiona.pl/images/2/26/Star.PNG",
            "Zbieranie ataków",
            "Zbieranie ataków - <b>dane graczy</b>. <br />Czekaj!"
          );
          let i = 0;
          for (const coord in villageData) {
            ...
            UI.InfoMessage(
              `Zbieram dane o graczach: <b>${++i}</b> na ${totalVillages}. <br /> Wioski z aktywnymi atakami: <b>${villageCount}</b>.`
            );
          }
          ...
          UI.Notification.show(
            "https://help.plemiona.pl/images/2/26/Star.PNG",
            "Zbieranie ataków",
            "Zbieranie ataków - <b>dane z wiosek</b>. <br />Czekaj!"
          );
          i = 0;
          for (const coord in villageData) {
            UI.InfoMessage(`Zbieram dane o wioskach: <b>${++i}</b>.`);
            const id = villageData[coord];
            await pause(200); // Introduce a 200ms pause before each request
            const result = await fetchVillageData(id, coord);
            for (const nickname in result) {
